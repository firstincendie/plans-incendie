    ...
    <div>
      ...
      {plans.map((p, i) => (
        <div key={i} className="tableau-plans-ligne">
          <div className="tp-num" style={{ textAlign: "center" }}>{i + 1}</div>
          ...
          <select className="tp-ori"  value={p.orientation}       onChange={e => updatePlan(i, "orientation", e.target.value)} style={sel}>{ORIENTATIONS.map(o => <option key={o}>{o}</option>)}</select>
          ...
          <button className="tp-del" onClick={() => supprimerLigne(i)} disabled={i === 0}
