...
export default function ListeCommandes() {
  ...
  // Toggle helper pour les headers de colonnes cliquables
  const toggleTri = (col) =>
  ...
  // Marquer en lue : (a) clear le marquage manuel s'il existe + (b) ajouter
  // auteurNom au lu_par de chaque message d'un autre auteur non encore lu.
  ...
  // Sélecteur de tri pour le mode cartes (mobile)
  const MobileSort = () => {
    const options = [
  ...
  return (
    <div onClick={() => { menuCmdId && setMenuCmdId(null); }}>
      ...
      {menuCmdId && menuRect && (() => {
        ...
        const top = spaceBelow >= 220 ? menuRect.bottom + 4 : menuRect.top - 4;
        const transform = spaceBelow >= 220 ? "none" : "translateY(-100%)";
        return (
          <div
            ...
            {/* Marquer en non lue / lue — mutuellement exclusifs selon l'état */}
            {hasNotif(c) ? (
              <button
