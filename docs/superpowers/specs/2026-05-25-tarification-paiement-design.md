...
**Produit :** Plans‑Incendie · **Date :** 2026‑05‑25 · **Version :** 1.5 · **Statut :** À valider

> Ce document remplace le brouillon de brainstorm précédent (facturation mensuelle / modèle linéaire), abandonné au profit du modèle décrit ici : **commande à formats de délai, paiement par carte à la validation, encaissement intégral par la société puis reversement aux dessinateurs sur facture**.
...
1. une **tarification par dessinateur** (grille de prix + tarif de déclinaison + formats de délai) ;
...
3. un **paiement par carte à la validation finale**, encaissé par la société (qui prélève une **commission** et reverse au dessinateur sur facture), avec **facture par commande** ;
...
**Inclus :** grille de prix par dessinateur, plans principaux & déclinaisons, formats de délai et SLA, pénalités, modifications, validation de prise en charge par le dessinateur, parcours de commande en 2 étapes, paiement carte + commission + reversement, facture par commande, filigrane et stockage privé, statistiques.
...
- **Dessinateur** (`dessinateur`) — réalise les plans ; définit sa grille et ses disponibilités (soumises à validation admin) ; valide la prise en charge ; **facture sa part à la société** et est réglé hors plateforme.
- **Admin / société** — valide les comptes, les grilles de tarifs et les disponibilités ; **encaisse**, émet les factures aux clients, prélève la commission, reverse aux dessinateurs ; supervise paiements et litiges.
...
- **Commission** : marge de la société, **ajoutée au‑dessus** du prix du dessinateur (modèle majoration) — **20 % avec minimum 1,50 €** par commande (cf. §11).
...
**Exemples (montant dessinateur HT) :**

- *1 plan d'intervention A3 + 4 plans d'évacuation A4 (déclinaisons)* → 6 € + 4 × 5 € = **26 €** (× coefficient de délai).
- *3 plans d'évacuation (1 principal A3 + 2 déclinaisons)* → 6 € + 2 × 5 € = **16 €**.
- *1 plan d'évacuation A3 seul, en 48h* → 6 € × 1,5 = **9 €** (un plan seul n'a pas de surcoût).
...
La **société facture le client à 20 %** (TVA obligatoire). Le **dessinateur facture sa part à la société hors plateforme** ; l'application ne gère **aucune TVA dessinateur**.
...
- Le **dessinateur** modifie sa grille / tarif de déclinaison / coefficients / **disponibilités**, puis **soumet une demande** ; les valeurs actives restent inchangées jusqu'à **approbation par l'admin**.
...
> **Règle de déclenchement (validée) :** le délai est évalué **à chaque étape** (ébauche, retouches, plans finaux), avec une **marge de tolérance de 20 %** du temps de l'étape avant qu'un dépassement compte comme retard. En cas de retard : **déclassement puis malus** (le malus s'applique une seule fois, selon le format commandé).
...
- **La société encaisse la totalité** via un **paiement Stripe standard** (pas de Stripe Connect, pas d'onboarding des dessinateurs). Elle **reverse ensuite au dessinateur sur facture** : la plateforme **calcule et affiche, par commande, la part due au dessinateur** (= son montant), que le dessinateur facture à la société.

### 11.2 Commission

- Modèle **majoration** : la grille du dessinateur = ce qu'il touche ; la société **ajoute sa commission par‑dessus**.
- **Commission = 20 % du montant dessinateur, avec un minimum de 1,50 € par commande.**
- Les **frais Stripe** (~0,25 € + ~1,5 % par paiement) sont à la **charge de la société** (couverts par la commission).

### 11.3 Montant final

```
Sous-total plans       = Σ prix_principal[type, format] + (nb déclinaisons × tarif_déclinaison)
Montant dessinateur HT = Sous-total plans × coefficient[format délai]
                         × (1 - malus) si retard (après déclassement)
                         + modifications post-validation éventuelles
Commission             = max(20 % × Montant dessinateur HT ; 1,50 €)
Montant client HT      = Montant dessinateur HT + Commission
TVA (20 %)             = Montant client HT × 0,20
Montant client TTC     = Montant client HT + TVA
```

*Exemple — 1 plan d'évacuation A3 seul, délai normal :* dessinateur **6 €** → commission max(1,20 ; 1,50) = **1,50 €** → client HT **7,50 €** → TVA **1,50 €** → **TTC 9,00 €**. La société reverse **6 €** au dessinateur sur sa facture.

### 11.4 Facture & flux

- La **société émet la facture** au client (TVA 20 %), **une facture par commande**.
- Le **dessinateur facture la société** (hors plateforme) pour sa part ; **aucune TVA dessinateur** n'est gérée dans l'application.
- Clients = **sociétés vérifiées** (SIREN) ; le paiement carte fournit la **preuve** de règlement.

### 11.5 Commande non aboutie
...
Côté **client** comme **dessinateur** : chiffre d'affaires / dépenses **par mois** et **par dessinateur**, historique des factures. Côté dessinateur, **part due / déjà reversée**.
...
**`profiles` (dessinateur) :** jours de travail, heure début/fin.

**`tarifs_dessinateur`** : `dessinateur_id` (PK), `tarif_declinaison`, `coef_24h`, `coef_48h`, `coef_72h`, `coef_normale`, `actif`.
...
- **Compte dessinateur** : onglets **Tarifs** (grille + déclinaison + coefficients), **Disponibilités** (→ validation admin), **Facturation/Stats** (part due / reversée).
- **Compte client** : onglet **Facturation/Stats** ; **téléchargement de l'ébauche filigranée**.
- **`GestionUtilisateurs`** (admin) : validation des **demandes de tarifs / disponibilités**, supervision paiements/factures, **réclamations**, suivi des reversements.
...
- **Paiement** : **Stripe standard** (paiement carte, la société encaisse tout) ; calcul de la **part dessinateur + commission** ; webhooks → **déblocage** ; reversement aux dessinateurs **sur facture** (hors plateforme).

### C. Points tranchés & résiduels

**Tranchés :** coefficient de délai (§5.5) ; déclenchement des pénalités = **étape par étape avec tolérance 20 %** (§8) ; structure juridique = **la société facture le client à 20 % de TVA, le dessinateur facture la société** (§11) ; commission = **majoration 20 %, minimum 1,50 €** (§11.2).

**Résiduels :** confirmer définitivement le **modèle de commission** (majoration retenu) ; **fixer les valeurs par défaut** des coefficients de délai et le **tarif de déclinaison de référence** ; cadrage comptable fin (numérotation des factures, mentions légales).
