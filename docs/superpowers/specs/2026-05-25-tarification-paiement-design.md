...
**Prise en charge par le dessinateur.** Après création, la commande attend la **validation de prise en charge**. Le dessinateur peut :

- **accepter** → la prise en charge **démarre le chrono** de la première ébauche ;
- **demander une correction au client** s'il estime la commande inadaptée (format inadapté, ou plan à requalifier en principal / déclinaison) → la commande retourne au client, puis est resoumise.
...
- **`ModalDetailCommande` / `DetailCommandeModal`** : **prise en charge** (accepter / demander correction format ou principal↔déclinaison) ; dépôt des plans finaux filigranés ; **paywall** ; affichage montant + chrono ; **journal du temps** ; **réclamation / demande admin**.
